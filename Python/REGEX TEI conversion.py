...
def replace_match(match):
    ...
    return '<PLACE subtype="inferred" type="{}" key="{}">€placeNameß{}{}€/placeNameß</PLACE>'.format(
        typeq,
        toponym["3"],
        match.group(1),
...
toponym_list = [
    ...
    {"1": r'América', "2": 'Territory', "3": 'generico'},
    {"1": r'Indias', "2": 'Territory', "3": 'generico'},
    {"1": r'Carolina', "2": 'Territory', "3": 'Provincia'},
    {"1": r'Georgia', "2": 'Territory', "3": 'Provincia'},
    {"1": r'Europa', "2": 'Territory', "3": 'generico'},
...
##    {
##        "search": r'(?P<front><list subtype="mixed".+?\r\n.*?)(?P<back>(<item>)(.+?)?(.+?)((.+?)?</item>\r\n))',
##        "replace": r'\g<front>µ\g<subtype>µµ\g<back>'
##    },
]
...
for typeq in type_list:
    for toponym in toponym_list:
            modified_content = re.sub(
                r'<'+typeq+'>(' + str(toponym["1"]) + ')( [A-Z][a-z]+)?</'+typeq+'>',
                replace_match,
                modified_content
            )
            original_content = modified_content
...
for role in role_list:
    modified_content = original_content
    modified_content = re.sub('(?P<b1>'+str(role["1"])+')' +'(?P<b2> del? ?l?a? )<Territory>'+'(?P<b3>[^<]+?)'+'</Territory>','\g<b1>\g<b2><PLACE subtype="inferred" type="Territory" key="'+str(role["2"])+'">€placeNameß\g<b3>€/placeNameß</PLACE>', modified_content)
    original_content = modified_content
# Search for each substring and count its
for substring in countstrings:
    substring_counts[substring] = len(re.findall(re.escape(substring), original_content))
...
replacement_patterns5 = [
    ...
    {"search": r'(<PLACE>)(<term[^<]*?type=")([^<]+?)(" gender="[^"]*?" key=")([^"]+?)(s?">[^<]+?</term>[^<]+?[€<]placeName type="\3"[ß>][^<]+?[€<]/placeName[ß>]</PLACE>)(?:,? ?y?e? d?e? ?[^<]{0,7}?)<\3>([^<]+?)</\3>',
     "replace": r'\1\2\3\4\5\6<PLACE subtype="inferred" type="\3" gender="x" key="\5">€placeName type="\3"ß\7€/placeNameß</PLACE>µ'},
    #Wrap third item in enumeration of toponyms of same type (after relative type resolution)
    {"search": r'<PLACE subtype="[^"]+?" type="([^<]+?)" gender="[^"]*?" key="([^<]+?)">€placeName type="\1"ß([^<]+?)€/placeNameß</PLACE>µ(,? ?y?e? d?e? ?[^<]{0,7}?)<\1>([^<]+?)</\1>',
...
for typeq in type_list:
    replacement_patterns6 = [
        ...
        {"search": r'µ([^<µ]+?)µ([^<µ]+?)µµ(,? ?y?e? d?e? ?[^<]{0,7})<\1>([^<]+?)</\1>',"replace": r'\3 <PLACE subtype="inferred" type="\1" key="\2" gender="x">€placeName type="\1"ß\4€/placeNameß</PLACE>µ\1µ\2µµ'}]
    while True:
        modified_content = original_content
